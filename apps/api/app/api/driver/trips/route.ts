...
const schema = z.object({
    action: z.enum(['view', 'history', 'today', 'pickup', 'pickup-all', 'dropoff', 'dropoff-all']),
...
export async function POST(req: Request) {
    ...
    if (action === 'today') {
        ...
        // get location info
        let locations = await db.selectFrom('location')
            .select([
                'latitude',
                'longitude',
                'created_at'
...
async function getParentEmailByTripId(trip_id: number) {
    let trip = await db.selectFrom('daily_ride')
        .leftJoin('ride', 'daily_ride.ride_id', 'ride.id')
    ...
    if (trip?.meta) {
        let when_bus_makes_home_drop_off = trip.meta?.notifications.when_bus_makes_home_drop_off;
        let when_bus_make_home_pickup = trip.meta?.notifications.when_bus_make_home_pickup;
        if (trip.kind == 'Pickup') {
