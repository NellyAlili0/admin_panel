  ...
  if (action === "today") {
    ...
    for (let trip of kind === "pickup" ? pickup : dropoff) {
      ...
      if (trip.status == "Active" && cordinates) {
        ...
        let distance = await mapping.getDistance({
          origin: cordinates?.latitude + "," + cordinates?.longitude,
          destination: pickupMeta.latitude + "," + pickupMeta.longitude,
        });
    ...
    if (parent) {
      notify.sendSingle({
        ...
        message:
          "Your child has been " +
          (kind === "pickup" ? "picked up" : "dropped off"),
        email: parent.email!,
      });
    }
    return Response.json(
